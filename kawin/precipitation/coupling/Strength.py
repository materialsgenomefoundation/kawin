...
class StrengthModel:
    ...
    def ssStrength(self, model, n):
        ...
        for i in range(len(model.elements)):
            if model.elements[i] in self.ssweights:
                val += self.ssweights[model.elements[i]]*model.data.composition[n,i]**self.ssexp
        return val
    ...
    def updateCoupledModel(self, model):
        ...
        self.solidStrength = np.append(self.solidStrength, [self.ssStrength(model, model.data.n)], axis=0)
    ...
    def plotPrecipitateStrengthOverTime(self, ax, model, phase = None, bounds = None, timeUnits = 's', strengthUnits = 'MPa', contribution = None, *args, **kwargs):
        ...
        contribution : None or str
            ...
            If str, will plot selected contribution or all contributions
                Options are: Coherency, Modulus, APB, SFE or Interfacial
        '''
        timeScale, timeLabel, bounds = _get_time_axis(model.data.time, timeUnits, bounds)

        self.plotPrecipitateStrengthOverX(ax, model.data.time*timeScale, self.rss, self.ls, phase, strengthUnits, contribution, *args, **kwargs)
    ...
    def plotStrength(self, ax, model, plotContributions = False, bounds = None, timeUnits = 's', strengthUnits = 'MPa', *args, **kwargs):
        ...
        sigma0 = self.sigma0 * np.ones(len(model.data.time))
        ...
        ax.plot(model.data.time*timeScale, total / yscale, *args, **kwargs)

        if plotContributions:
            ax.plot(model.data.time*timeScale, sigma0 / yscale, *args, **kwargs)
            ax.plot(model.data.time*timeScale, ssStrength / yscale, *args, **kwargs)
            ax.plot(model.data.time*timeScale, precStrength / yscale, *args, **kwargs)
