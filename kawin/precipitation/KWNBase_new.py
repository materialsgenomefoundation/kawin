...
class PrecipitateBase(GenericModel):
    ...
    def _incubationNonIsothermal(self, t, p, Z, betas):
        ...
        LHS = 1 / (self.theta[p] * Z**2 * (T / self.temperature[startIndex:]))

        RHS = np.cumsum(self.betas[startIndex+1:self.n,p] * (self.time[startIndex+1:self.n] - self.time[startIndex:self.n-1]))
        if len(RHS) == 0:
